...
public class EmailVerificacionServlet extends HttpServlet {

	private static final String TYPE_RESPONSE = "application/json";
	private static final String EMAIL = "email";
	private static final Logger LOG = Logger.getLogger(EmailVerificacionServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		String curpReq = "";
		String emailReq = "";
		String metodo = "consultaInfoVerificacionEmail";
		String urlFinal = "consultaVerificacionEmail";
		LOG.debug("***Entro en EmailVerificacionServlet servlet ****");
		...
		try {
			...
			if (respuestaValidaciones.validaRfc(rfcReq) && respuestaValidaciones.validaCurp(curpReq)
				...
				LOG.debug("***Final de consulta VerificacionEmail ****");
				...
				if (!data.isEmpty()) {
				    ...
				    json2.put("dataVerificacionEmail", data);
					response.getWriter().write(json2.toString());
				} else {
					response.setContentType(TYPE_RESPONSE);
					response.getWriter().write(json2.toString());
					LOG.debug("*** Sin informacion EmailVerificacion  ****");					
				}

			} else {
				response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				LOG.debug("*** Error Request  EmailVerificacion  ****");
				response.getWriter().write("EmailVerificacionErro,error");
			}

		} catch (Exception exc) {
			LOG.debug("*** Error  al obtener EmailVerificacion  ****", exc);
		}
